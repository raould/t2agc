...
(class Task
  ;; Identity
  (field (id) 0)              ; Unique integer PID
  (field (name) nil)          ; Optional symbolic name for debugging
  
  ;; Execution state
  (field (priority) "normal")  ; "critical" | "high" | "normal" | "low" | "idle"
  (field (status) "runnable")  ; "runnable" | "waiting" | "done" | "crashed"
  (field (gen) nil)           ; Generator function* instance
  (field (budget) 100)        ; Reductions remaining in current slice
  (field (initial-budget) 100) ; Reset value for budget
  
  ;; Mailbox
  (field (mailbox) (array))                 ; Array of messages (FIFO)
  (field (mailbox-max) 1000)           ; Max mailbox size
  (field (mailbox-overflow-policy) "drop-oldest") ; "drop-oldest" | "drop-newest" | "reject" | "escalate" | "block-sender"
  (field (waiting-patterns) nil)       ; Patterns this task is waiting for (when status = "waiting")
  
  ;; Capabilities & Effects
  (field (capabilities) (make-set))    ; Set of capability tokens
  
  ;; Histories (Ring Buffers)
  (field (history-effects) (make-ring-buffer 100))      ; Last 100 effect events
  (field (history-exceptional) (make-ring-buffer 50))   ; Last 50 exceptional events
  (field (history-critical) (make-ring-buffer 20))      ; Last 20 critical events
  
  ;; Statistics (for diagnostics)
  (field (created-at) (timestamp))
  (field (total-reductions) 0)
  (field (total-messages-sent) 0)
  (field (total-messages-received) 0)
  (field (mailbox-scan-count) 0)          ; Number of selective receive scans
  (field (total-mailbox-scan-operations) 0) ; Total messages scanned
)
```
...
(class Scheduler
  ...
  (field (current-task) nil)
  (field (tick-count) 0)
  (field (running) false)
  
  ;; System monitoring (for overload detection)
  (field (total-run-queue-length) 0)
  (field (avg-slice-duration) 0)      ; EMA of slice execution time
  (field (slice-duration-samples) (array))
  (field (overload-threshold-queue-length) 1000)
  (field (overload-threshold-slice-ms) 50)
  
  ;; Global histories
  (field (orchestrator-history) (make-ring-buffer 200))
  
  ;; AGC Code emission
  (field (agc-codes-emitted) (array))
  
  ;; Priority order for scheduling
  (field (priority-order) (array "critical" "high" "normal" "low" "idle"))
  ...
  (method handle-primitive (task primitive)
  ...
  (method record-slice-duration (duration)
...
```t2
(defn spawn (generator-fn args priority capabilities)
...
```t2
(method handle-receive (task patterns)
...
```t2
(method check-mailbox-health (task)
...
```t2
(defn match-pattern (pattern value bindings)
...
```t2
(method handle-selective-receive (task patterns)
...
```t2
(defn try-match-patterns (msg patterns)
...
```t2
(class Capability
  (field (id) (generate-uuid))
  (field (type) "log")  ; "log" | "io" | "timer" | "random" | "shared-blob"
  (field (operations) (array "info" "warn" "error"))  ; Allowed operations
...
```t2
(defn effect (capability operation & args)
...
```t2
(method handle-effect (task capability operation args)
...
(method dispatch-effect (capability operation args)
...
(method effect-log (operation args)
  (match operation
...
(method effect-io (operation args capability)
  (match operation
    ("fetch" 
      (let (url) args)
      ;; Check allowed hosts
...
(method effect-timer (operation args)
  (match operation
    ("sleep"
      (let (ms) args)
...
(method effect-random (operation args)
  (match operation
    ("next" () (Math.random))
...
```t2
(class AGCEvent
  (field (code) "")           ; e.g. "AGC-M010"
  (field (message) "")        ; Human-readable description
  (field (timestamp) 0)       ; When emitted
  (field (task-id) nil)       ; Which task (if applicable)
  (field (context) {})        ; Additional data
)
...
(do
  ...
  (class __Counter
    (field (value) 0)
    (field (__opaque-tag) (Symbol "Counter")))
  
  ;; Constructor
  (defn make-counter (value)
    (let instance (new __Counter))
    (set! (. instance value) value)
    instance)
  
  ;; Accessor
  (defn counter-value (counter-instance)
