...
;; Expands to:
(def counter_fn 
  (generator_function (initial_value cap_log)
...
;; When spawned:
(let counter_pid (spawn counter_fn (array 0 my_cap_log) "normal" (make_set)))
;; Additional: configure mailbox on the task
(let task (map_get scheduler.tasks counter_pid))
